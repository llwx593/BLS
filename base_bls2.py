...
class BaseBls:
    ...
    def train_forward(self, train_x, train_y):
        ...
        self.test_output = np.zeros([train_x.shape[0], self.N2 * self.N1])
        self.test_output2 = np.zeros([train_x.shape[0], self.N2 * self.N1])
        for i in range(self.N2):
            ...
            self.test_output[:, self.N1*i:self.N1*(i+1)] = preprocess_feature
            ...
            self.test_output2[:, self.N1*i:self.N1*(i+1)] = feature_node_output
        ...
        print(self.feature_weight_list)
        print("before standard")
        print(self.test_output2)
